...
export async function Nav() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-[color:var(--card-border)] bg-[color:var(--bg)]/70 backdrop-blur-md">
        ...
        <nav className="flex items-center gap-1">
          {links.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="rounded-full px-4 py-2 font-[family-name:var(--font-display)] text-[13px] tracking-wide text-[color:var(--ink-dim)] transition-colors hover:bg-black/[0.04] hover:text-[color:var(--ink)]"
            >
              {link.label}
            </Link>
          ))}
