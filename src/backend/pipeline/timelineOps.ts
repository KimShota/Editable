...
export const TimelineOpSchema = z.discriminatedUnion("type", [
  ...
  z.object({
    ...
    track: z.enum(["video", "overlay", "sfx", "transition", "music"]),
...
const applyMove = (edl: Edl, op: Extract<TimelineOp, { type: "move" }>): void => {
  if (op.track === "overlay") {
    ...
    const duration = clip.tlOutSec - clip.tlInSec;
    clip.tlInSec = op.tlInSec;
    clip.tlOutSec = op.tlInSec + duration;
    return;
  }
  if (op.track === "sfx") {
  ...
  if (op.track === "captions") {
    ...
    // A rigid shift: the words move with the group, so the per-word
    // highlight timing stays aligned with when the group is on screen.
    const delta = op.tlInSec - clip.tlInSec;
    clip.tlInSec += delta;
    clip.tlOutSec += delta;
